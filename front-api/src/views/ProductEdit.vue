<template >
  <form @submit.prevent = "updateProduct" class = "bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" >
    ...
    <div class = "flex items-center justify-between" >
      <button type = "submit"
              class = "bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" >Mettre à jour le produit</button >
    </div >
  </form >
</template >
...
export default {
  data() {
    return {
      ...
      selectedImage: null // New data property for the selected image
    };
  },
  async created() {
  ...
  methods: {
    onFileChange(e) {
      ...
      console.log(this.selectedImage); // Log the selected image
    },
    async updateProduct() {
      const productId = this.$route.params.id;
      try {
